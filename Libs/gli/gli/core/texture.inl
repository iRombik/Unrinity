...
namespace gli
{
    ...
    size_t texture::mip_level_width(size_t levelId) {
        GLI_ASSERT(!this->empty());
		return this->Storage->extent(levelId).x;
    }

    size_t texture::mip_level_height(size_t levelId) {
        GLI_ASSERT(!this->empty());
		return this->Storage->extent(levelId).y;
    }

    size_t texture::mip_level_depth(size_t levelId) {
        GLI_ASSERT(!this->empty());
        return this->Storage->extent(levelId).z;
    }
